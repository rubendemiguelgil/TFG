function [mapa_conjunto,x_ref,y_ref] = visibilidad_conjunto_optima_algo(clutter,prf_array,n_para_deteccion,fi_apuntamiento_deg)
%esta funcion requiere que el clutter se le pase como parametro
%esta version no corrige las prf (usada para el algoritmo)
n_prf=size(prf_array,2);

for i1=1:n_prf
   [mapa,x,y]=mapa_visibilidad_algo(clutter,prf_array(i1),fi_apuntamiento_deg); 
