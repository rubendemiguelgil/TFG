function [mapa,x,y] = mapa_visibilidad_algo(clutter,prf_input,fi_apuntamiento_deg)
%version no corrige la prf (se utiliza en el algoritmo, donde tendremos mas
%flexibilidad para escoger la prf por disponer de una resolucion mayor que
%la celda)
...
nf=size(clutter,1)/2; %justificar (256=positivo mas negativo)
...
n_filtros_dft=128;
n_range_bins_sol=ceil(1/(prf_input*t_pulso_sol_comprimido));%ponemos siempre una celda mas al ser la prf diferente de un n entero de celdas
prf=prf_input;
tot_2=n_filtros_dft/prf;

r_unambiguo=3*10^8/(2*prf);
res_dist_sol=t_pulso_sol_comprimido*3e8/2;
...
for i1=1:size(visibilidad,2)
     ...
     p_rec=(P_avg_2*(ganancia(0,0))^4*rcs_blanco*lambda^2)/((4*pi)^3*rango^4*perdidas);
